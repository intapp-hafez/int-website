...
function validate(input: { learnerEmail: string; staffEmail: string; trainingId?: string }) {
  ...
  if (!EMAIL_RE.test(learnerEmail)) throw new Error("Enter a valid learner email");
  if (!EMAIL_RE.test(staffEmail)) throw new Error("Enter a valid trainer/manager email");
