...
let sql = '';
let tsMap = 'export const partnerDescriptions: Record<string, { en: string, ar: string }> = {\n';
...
partners.forEach(p => {
  ...
  sql += `UPDATE partners SET description_en = '${safeEn}', description_ar = '${safeAr}' WHERE id = '${p.id}' OR name_en = '${p.nameEn}';\n`;
...
fs.writeFileSync('src/data/partner-descriptions.ts', tsMap);
fs.appendFileSync('supabase/migrations/20260820000002_add_descriptions_to_partners.sql', sql);
console.log('Success');
